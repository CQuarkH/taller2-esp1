import { getLifeBarColor, stopGameSoundtrack, playGameSoundtrack, playGameOverSound } from "./utils.js";
...
export const controlPlayers = (player1, player2) => {
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if (event.key === player1.attackKey && checkCollision(player1.getHitbox(), player2.getHitbox())) {
            player1.attack(player2);
            player2.flashDamageColor();
            updatePlayerLife(player2);
        }

        if (event.key === player2.attackKey && checkCollision(player1.getHitbox(), player2.getHitbox())) {
